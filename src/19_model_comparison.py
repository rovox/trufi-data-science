...
def main() -> None:
    ...
    for name in FITTED_MODELS:
        ...
        predictions["test"][name] = np.clip(model.predict(X_test), 0, None)
        print(f"  Loaded and scored: {name}")

    # Naive baselines require no model object — they are pre-computed columns.
    predictions["train"]["baseline_naive"] = train_df.select("lag1_n_queries_orig").to_numpy().ravel()
    predictions["test"]["baseline_naive"] = test_df.select("lag1_n_queries_orig").to_numpy().ravel()
    predictions["train"]["baseline_rolling4"] = train_df.select("roll_mean4_n_queries_orig").to_numpy().ravel()
    predictions["test"]["baseline_rolling4"] = test_df.select("roll_mean4_n_queries_orig").to_numpy().ravel()
