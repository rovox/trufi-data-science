...
def latlon_to_utm(lat: np.ndarray, lon: np.ndarray) -> tuple[np.ndarray, np.ndarray]:
    """Convert lat/lon to UTM coordinates (approximate for Cochabamba).

    Uses a simplified UTM projection centered on Cochabamba.
    Good enough for distance calculations within the metro area.
    """
