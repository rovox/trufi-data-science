...
Important: The 7-week data gap (2024-03-11 to 2024-04-22) is within the
training period, not at the split boundary.
...
def compute_split_statistics(
    ...
    train_stats = {
        ...
        "date_range": (
            train_df.select([pl.col("year").min(), pl.col("week").min()]).to_dicts()[0],
            train_df.select([pl.col("year").max(), pl.col("week").max()]).to_dicts()[0],
        ),
    }
    ...
    test_stats = {
        ...
        "date_range": (
            test_df.select([pl.col("year").min(), pl.col("week").min()]).to_dicts()[0],
            test_df.select([pl.col("year").max(), pl.col("week").max()]).to_dicts()[0],
        ),
    }
